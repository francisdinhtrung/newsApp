...
protocol NewsProtocol {
    func fetchTL(_ params: [String: Any]) -> Observable<[ArticleViewModel]>
    
    func fetchEveryThing(_ params: [String: Any]) -> Observable<[ArticleViewModel]>
}
...
struct NewsService : NewsProtocol {
    ...
    func fetchTL(_ params: [String : Any]) -> Observable<[ArticleViewModel]> {
        return self.provider.rx.request(.fetchTopHeadlines(params)).map(AppPaging.self)
            .asObservable()
            .map { $0.articles.map { ArticleViewModel(article: $0) } }
    }
